...
const Seat = ({
  ...
  // console.log(isOnward, "seatisOnward");
  ...
  let planeDetails =
    airlineSeatMap?.seatMap?.[planeNo]?.Air_RetrieveSeatMapReply
      ?.seatmapInformation;
  console.log(planeDetails, "airlineSeatMap");
  ...
  const handleSeatClick = (seat, seatNumber, planeNo, seatOccupation) => {
    ...
    setToolTip(true);
    // console.log(SEATES, "seatessssss");
    ...
    if (seat === "aisle" || seatOccupation) {
      ...
      selectedSeat[planeNo].push({
        ...
        currency: foundObject?.currency || "INR",
        planeNo: planeNo || 0,
        AirlineCode:
          planeDetails?.flightDateInformation?.companyDetails?.marketingCompany,
        FlightNumber:
          planeDetails?.flightDateInformation?.flightIdentification
            ?.flightNumber,
        CraftType: planeDetails?.equipmentInformation?.iataAircraftTypeCode,
        Origin:
          planeDetails?.flightDateInformation?.boardPointDetails
            ?.trueLocationId,
        Destination:
          planeDetails?.flightDateInformation?.offpointDetails?.trueLocationId,
        ...
        // Deck: "Number",
        Currency: foundObject?.currency || "INR",
        Price: foundObject?.amount || 0,
      });
    }
    //  else if (selectedSeat?.[planeNo]?.length === traveller) {
    //   selectedSeat[planeNo].shift();
  ...
  // console.log(traveller, "traveller", adultCount, childCount);
...
const Plane = ({ data, planeNo, isOnward }) => {
...
function FlightLayoutAMD({ data, isOnward }) {
  const dispatch = useDispatch();
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  //   const adultCount = queryParams.get("adult");
  ...
  useEffect(() => {
    if (data !== null) {
      ...
      if (isOnward) {
        dispatch(setDefaultSeatOccupationOnward(defaultSeatOccupation));
      } else {
        dispatch(setDefaultSeatOccupationReturn(defaultSeatOccupation));
      }
    }
  }, [data]);
  ...
  useEffect(() => {
    ...
    //   reducerState?.airlineSeatMapNew?.onward?.number_of_airline,
    //   "reducerState?.airlineSeatMapNew?.onward?.number_of_airline"
    // );
    let numberOfAirline = isOnward
      ? reducerState?.airlineSeatMapNew?.onward?.number_of_airline?.no
      : reducerState?.airlineSeatMapNew?.return?.number_of_airline?.no;
    // console.log(numberOfAirline, "numberOfAirline1117");
    if (data && data.length >= numberOfAirline) {
      ...
      if (isOnward) {
        dispatch(setAirlineAmountOnward(data2)); // Dispatch the full processed array
      } else {
        dispatch(setAirlineAmountReturn(data2)); // Dispatch the full processed array
      }
    }
  }, [data, dispatch]);
                                ...
                                <p>
                                  {
                                    findAirportByCode(
                                      d?.Air_RetrieveSeatMapReply
                                        ?.seatmapInformation
                                        ?.flightDateInformation?.offpointDetails
                                        ?.trueLocationId
                                    )?.name
                                  }
                                </p>
...
export default FlightLayoutAMD;
